  ...
  render() {
    return (
      <Input
        ...
        addonAfter={<Button onClick={this.props.add}>新增</Button>}
      ></Input>
    );
  }
}
  ...
  render() {
    return (
      <Row
        gutter={16}
        className={
          this.props.item.status === 0 ? "item--todo" : "item--finished"
        }
      >
        ...
        <Col className="gutter-row" span={6}>
          {this.props.item.status === 0 && (
            <Button onClick={() => this.props.finish(this.props.index)}>
              完成
            </Button>
          )}
          <Button onClick={this.props.del.bind(this, this.props.index)}>
            删除
          </Button>
        </Col>
      </Row>
...
class BaseList extends React.Component {
  render() {
    return this.props.list.map((item, index) => (
      <BaseItem item={item} {...this.props} key={index} index={index} />
    ));
  }
}
...
function TodoList({ addItem, deleteItem, changeItemStatus, list }) {
  ...
  return (
    <>
      ...
      <BaseList
        list={
          status === null ? list : list.filter(item => item.status === status)
        }
        key={status}
        finish={index => {
